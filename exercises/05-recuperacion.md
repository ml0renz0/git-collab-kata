...
## Parte A — Recuperar el árbol de trabajo y el stage
...
- ¿Por qué conviene probar primero `git clean -n` antes de ejecutar `git clean -fd`?
...
1. Ejecuta una simulación de limpieza para comprobar qué borraría Git:
...
## Parte B — Reflog y revert en recuperación compartida
...
> Aviso de seguridad: este `reset --hard` es una simulación local para aprender a rescatar trabajo. No empujes `$MAIN` después del reset; crea la rama de rescate desde el hash recuperado y abre PR desde esa rama.
...
- Completa la parte A como práctica guiada con `docs/recovery.md`, conservando `app/calculator.py`, usando `git clean` de forma acotada y sin subir `debug.conf`.
