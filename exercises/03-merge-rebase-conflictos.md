...
1. Asegúrate de que `divide` existe en `app/calculator.py`. Si no existe, integra primero el resultado del ejercicio 2A en tu rama `$MAIN`.
...
1. Actualiza tu rama local `$MAIN`.

---

## Preparación — Dos ramas que tocarán la misma función

Antes de implementar nada para el ejercicio 3B, crea dos ramas desde el mismo punto de partida. Esto hará que una rama quede desactualizada cuando la otra se integre primero.
...
## Ejercicio 3B — Rebase con conflicto manual
...
1. Persona B trae los cambios remotos.
...
- Completa el ejercicio 3A con `divide`, gestión explícita de división por cero mediante `ZeroDivisionError` y un test que incluya `zero`.
- Completa el ejercicio 3B resolviendo el conflicto manual en `add`.
