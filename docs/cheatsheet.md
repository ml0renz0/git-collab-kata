...
git merge --no-ff "$BRANCH"                     # crear merge commit aunque haya fast-forward
git merge --abort                               # cancelar un merge con conflictos
git log --oneline --graph --decorate "$MAIN"..HEAD    # ver la forma de la rama
```
...
git clean -n -- archivo                # simular limpieza de archivos sin seguimiento
git clean -fd -- archivo               # borrar archivos sin seguimiento de forma acotada
...
git revert -m 1 <merge-commit>         # revertir un merge conservando el primer padre
```

## Releases

```bash
git tag -a "v1.0.0-${USER_ID}" -m "Release v1.0.0 ${USER_ID}"  # crear tag anotado
git push origin "v1.0.0-${USER_ID}"                            # subir tag concreto
git show "v1.0.0-${USER_ID}"                                   # inspeccionar tag y commit
```

## Mantenimiento

```bash
git worktree add ../ruta "$RELEASE"         # abrir otra rama en otro directorio
git worktree list                           # listar worktrees activos
git worktree remove ../ruta                 # eliminar un worktree ya cerrado
git cherry-pick -x <commit>                 # aplicar un commit conservando referencia origen
```
...
git range-diff "origin/$MAIN" "origin/$FEATURE" HEAD    # comparar una rama reescrita con su versión anterior
...
git log -S "texto"         # buscar commits que cambian apariciones de texto
git log -G "patrón"        # buscar commits cuyas líneas cambiadas encajan con patrón
git bisect start           # iniciar búsqueda binaria de commit defectuoso
git bisect bad             # marcar el commit actual como defectuoso
git bisect good <commit>   # marcar un commit anterior como correcto
git bisect run pytest -q   # automatizar la búsqueda con tests
git bisect reset           # volver al estado anterior al bisect
```
